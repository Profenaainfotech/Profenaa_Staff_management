...
import { ArrowRight, CalendarClock, Cpu } from "lucide-react";
...
export default function MyTechnologyProjects({ tasks = [], onOpenTasks }) {
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500">Work the administrator has allocated to you, by domain.</p>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {mine.map((t) => (
          <article key={t._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <WorkItemList items={t.workItems} domains={t.domains} className="mt-4 rounded-xl bg-slate-50 p-3" />
          </article>
        ))}
      </div>
    </section>
